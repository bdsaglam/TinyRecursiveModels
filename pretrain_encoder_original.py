"""
Training script for encoder-based TRM - ORIGINAL TRAINING MODE.

This script implements ORIGINAL TRM training dynamics:
...
Key differences from pretrain_encoder.py (online learning):
1. Single forward per batch (no ACT loop)
2. Carry persists in train_state (not reset each batch)
3. Uses etrm_original.py model with _forward_train_original()
...
def train_batch_encoder_original(
    config: PretrainEncoderConfig,
...
@hydra.main(config_path="config", config_name="cfg_pretrain_encoder_original", version_base=None)
