...
All three encoder architectures achieve 0\% Pass@1 on held-out puzzles despite substantial training accuracy, indicating a complete generalization failure.
...
\begin{enumerate}
  \item Complete generalization failure: all ETRM variants obtain 0\% Pass@1 on held-out puzzles despite 41--79\% training accuracy.
  \item Encoder collapse explains the failure: encoders produce near-constant outputs across different puzzles.
  \item Architecture-agnostic failure: feedforward, variational, and iterative encoders all fail similarly.
  \item Training-test gap: the model memorizes training puzzle mappings rather than extracting generalizable rules.
  \item Contrast with TRM: TRM achieves $\sim$37\% Pass@1 at comparable training time by memorizing embeddings; the decoder can solve puzzles when given appropriate task context.
\end{enumerate}

Replacing learned embeddings with a demonstration encoder is a natural idea for enabling generalization, but our experiments show this is substantially harder than expected: the encoder consistently collapses to constant outputs rather than learning to extract transformation rules. Understanding and preventing this collapse is essential for progress on encoder-based few-shot program induction.
