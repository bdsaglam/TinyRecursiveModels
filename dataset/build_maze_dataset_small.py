"""
Build a small subset of the maze dataset for quick verification training.
This reduces both the number of examples and augmentation for faster iteration.
"""
...
class DataProcessConfig(BaseModel):
    ...
    output_dir: str = "data/maze-30x30-hard-1k-small"

    subsample_size: int = 100  # Only use 100 examples instead of 1000
    aug: bool = False  # No augmentation for quick testing
...
def convert_subset(set_name: str, config: DataProcessConfig):
    ...
    for inp, out in zip(tqdm(inputs), labels):
        # Dihedral transformations for augmentation (skip for quick testing)
...
def preprocess_data(config: DataProcessConfig):
    print("Building small maze dataset for quick verification...")
    print(f"  Subsample size: {config.subsample_size}")
    print(f"  Augmentation: {config.aug}")
